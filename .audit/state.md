...
- **Deriva repo↔producción detectada:** `assets/images/logo.png` es 7.423 B en el repo (escala de grises + alfa) y 10.079 B en producción (RGBA). Ambos 240×240 y ambos cumplen el DoD, pero **no son el mismo archivo**. Conviene alinearlos en el próximo despliegue.
- **Tareas: 22 — 10 COMPLETED, 2 NOT DONE, 1 PARTIALLY_COMPLETED, 1 IMPLEMENTED_PENDING_VALIDATION, 2 READY, 7 BLOCKED.**
- **Próximas acciones ejecutables:** TASK-0011 (`?v=`, PERF-002 — ahora más relevante: al renombrar a `main.min.css` la caché se invalidó una vez, pero las siguientes ediciones del mismo archivo volverán a chocar con los 7 días de `max-age`), TASK-0010 (fallback sin JS, AEO-001), TASK-0021.
...
- **Filtrado de eventos por ciudad cerrado (2026-07-22, ADR 0022).** Derogada la condición de revisión que preveía `/eventos/ciudad/{ciudad}`: no se crean URLs de filtro por ciudad con ningún volumen. Contradecía el escalado por año ya definido en `docs/03` §3, el umbral era de cantidad en vez de contenido, y es navegación facetada. Se mantiene `event_city` como taxonomía sin archivo público y los encuentros dentro de `/sanghas/{ciudad}`. Afecta a **TASK-0020**, que ya no debe contemplar esa salida.
