...
- Repo layout: **monorepo** (ADR 0014, since Fase 3 WU-01). Deployable static site lives in `static/`. `wordpress/` holds first-party plugin/theme trees: `camino-del-dharma-core` v0.5.0 owns the domain models (WU-05), the migration pipeline (WU-06), the field-scoped content conversion `migrate convert` (WU-07/WU-08A) and the share/calendar data of WU-08A (CPT `event` + non-public `event_type`/`event_city`, `gallery_album`, CPT `blog_author` + `authors` relation with publication guard, request-time event status in `America/Bogota`, calendar data, `cdd_core_event_calendar_payload()` as the single source of the calendar dialog **and** the generated `/eventos/ical/{slug}.ics`, editable `share_whatsapp`/`share_x`/`share_threads` meta; TDD kit from WU-03); the FSE theme `camino-del-dharma` v0.3.0 has the **real views** since WU-07 and the **ported behavior** since WU-08A: 16 block templates, header/footer parts via PHP patterns, 13 dynamic blocks (events calendar with byte parity against the published grid, listing with compact past cards, home featured note, ADR 0037 bylines, native per-album galleries, share and add-to-calendar triggers), full static CSS ported to presets, self-hosted fontFaces, native lightbox, share/calendar dialogs enqueued per view.
...
- Deploy: manual ZIP from `static/` (ADR 0015). WordPress: **Fase 3 started** (WU-00–WU-08A done; next is **WU-08B**). Migration pipeline: versioned `migration/payload.json` (VERSION 1.0.35; live parity verified, delta 0) + `wp cdd-core migrate validate|plan|import|verify|convert` and `seed` (dry-run by default, create-missing-only; `convert` takes an optional `--payload` to seed the share copy add-only). Local env has the content imported **and converted** (dynamic home note, native album galleries, OWN-016 links, native mantra audio, share meta). Durable state: `.audit/fase3-execution-state.md`.
