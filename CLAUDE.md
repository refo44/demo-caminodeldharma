...
- Repo layout: **monorepo** (ADR 0014, since Fase 3 WU-01). Deployable static site lives in `static/`. `wordpress/` holds first-party plugin/theme trees: `camino-del-dharma-core` v0.2.0 owns the domain models since WU-05 (CPT `event` + non-public `event_type`/`event_city`, `gallery_album`, CPT `blog_author` + `authors` relation with publication guard, request-time event status in `America/Bogota`, calendar data, generated `/eventos/ical/{slug}.ics`; TDD kit from WU-03: Composer + PHPUnit/wp-phpunit + PHPCS/WPCS); the FSE theme `camino-del-dharma` is scaffolded (WU-04: `theme.json` v3 visual-token baseline mirroring the static `:root`, minimal `templates/index.html` + parts, `functions.php` bootstrap; real templates come in later WUs).
...
- Deploy: manual ZIP from `static/` (ADR 0015). WordPress: **Fase 3 started** (WU-00–WU-05 done; next unit is WU-06, extractor/payload/WP-CLI importer/reconciliation). Durable state: `.audit/fase3-execution-state.md`.
