...
- Deploy: manual ZIP from `static/` (ADR 0015). WordPress: **Fase 3**
  ...
  is still no `add_meta_box`). **META-002–META-005 done** (plugin 0.7.5, [issue
  #19](https://github.com/refo44/demo-caminodeldharma/issues/19), OWN-035:
  Gutenberg SEO / event panels for `post`/`page`/`event`/`blog_author`, sync
  through `core/editor`, publish-time `seo_description` backfill; still no
  `add_meta_box`, guard unchanged). **OWN-035:** the owner widened pre-staging
  on 2026-09-02 — with D-02/D-03/D-04 (issues #10–#12), #18 and #19 all on
  `main`, the **pre-staging code is complete**. That Hostinger WordPress now
  serves `https://caminodeldharma.org/` (ADR 0047 / OWN-036, 2026-09-26).
  `WP_ENVIRONMENT_TYPE` is `production` and `blog_public` is `1`. Do not create
  a second WordPress. The palegreen `public_html` is no longer the served
  rollback; do not delete the hPanel site entry or the private backups. Do not
  implement #20 outside its own PR. **OWN-020 / D-08** is implemented (plugin
  0.7.7, [issue #5](https://github.com/refo44/demo-caminodeldharma/issues/5)).
  Contact-form email delivery remains unverified and the form is hidden (ADR
  0045 / OWN-033). Durable state: `.audit/fase3-execution-state.md`. Migration
  pipeline: versioned `migration/payload.json` (VERSION 1.0.35; live parity
  verified, delta 0) + `wp cdd-core migrate validate|plan|import|verify|convert`
  and `seed` (dry-run by default, create-missing-only; `convert` takes an
  optional `--payload` to seed the share and SEO copy add-only; the payload
  carries a per-object `seo` object and an uncounted `site` section). Local env
  has the content imported **and converted** (dynamic home note, native album
  galleries, OWN-016 links, native mantra audio, share and SEO meta).
