...
**ADR (decisiones arquitectónicas):** `adr/README.md` (0001–0033). **Migración:** `contrato-migracion-static-wordpress.md` (contrato de aceptación, ADR 0032), `matriz-migracion-static-wordpress.md` (una fila por URL pública), `cutover-checklist-wordpress.md`, `migracion-static-wordpress.md` (ledger de diferencias, no el contrato). **Entorno local Docker:** `docker-wordpress-playbook.md` — arquitectura, gotchas y checklist para el entorno local de WordPress (ADR 0023); el compose **aún no está en el repo**. **Playbook de migración:** `playbook-migracion-static-wordpress.md` — WU, jerarquía de fuentes, QA en 4 niveles, invariantes, anti-patrones; el contrato de completitud es ADR 0032. **Archivo (respaldos):** `archive/contacto-formulario-estatico/` — snapshot de la página `/contacto` y estilos del formulario para restauración post-WordPress o tras cambios del estático. No llevan prefijo numérico en `00` para no romper la regla de dependencias lineal; se referencian desde `17-orden-implementacion`.
