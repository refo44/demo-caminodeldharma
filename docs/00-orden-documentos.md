...
**ADR (decisiones arquitectónicas):** `adr/README.md` (0001–0028). **Migración:** `migracion-static-wordpress.md`. **Entorno local Docker:** `docker-wordpress-playbook.md` — arquitectura, gotchas y checklist para el entorno local de WordPress (ADR 0023). **Playbook de migración:** `playbook-migracion-static-wordpress.md` — aprendizajes generalizables de la migración static→WordPress (WU con estado durable, jerarquía de fuentes de verdad, migración de contenido, matriz de cobertura, QA en 4 niveles, invariantes, anti-patrones); mapea cada aprendizaje al estado real del proyecto. **Archivo (respaldos):** `archive/contacto-formulario-estatico/` — snapshot de la página `/contacto` y estilos del formulario para restauración post-WordPress o tras cambios del estático. No llevan prefijo numérico en `00` para no romper la regla de dependencias lineal; se referencian desde `17-orden-implementacion`.
