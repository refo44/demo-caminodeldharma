...
| OWN-025 | 2026-09-01 | **D-03 cerrado.** Feeds nativos **404** reales; sin `rel=alternate` RSS. ADR 0044. Código **implementado** en plugin 0.7.3 ([#11](https://github.com/refo44/demo-caminodeldharma/issues/11)). RSS futuro: POST-010. |
...
| OWN-028 | 2026-09-01 | **D-06 cerrado: A.** Aceptar comillas `wptexturize` (`«…»` vs rectas del estático). |
| OWN-029 | 2026-09-01 | **D-07 cerrado: A.** Aceptar `core/audio` sin la línea «Tu navegador no permite…». |
| OWN-030 | 2026-09-01 | **D-11 cerrado: B.** El `count = 0` de álbumes en wp-admin se deja en el corte. Después de WP en el dominio canónico: conteo real o nota de admin. POST-009 / [#13](https://github.com/refo44/demo-caminodeldharma/issues/13). |
| OWN-031 | 2026-09-01 | **D-12 cerrado: A.** Conservar `lang="es-CO"`. Delta deliberado frente a `es` del estático. |
...
Hostinger ya existe y es el WordPress del corte (OWN-036 / ADR 0046); no se crea otro. D-08
...
| META-002 + META-003 | **Cumplidas** 2026-09-03 (plugin 0.7.5, [#19](https://github.com/refo44/demo-caminodeldharma/issues/19)): «SEO y buscadores» (cabeza) para los cuatro tipos y «Datos del evento (schema.org)» solo para `event`, fusionadas en una unidad | `PluginDocumentSettingPanel` nativo, sync por `core/editor`, sin `wp-api-fetch`; **sin** `add_meta_box`. `seo_jsonld_extra` y paneles `share_*` diferidos. Backfill de `seo_description` al publicar, add-only, nunca bajo WP-CLI. |
| META-004 | **Cumplida** 2026-09-03 (plugin 0.7.5): `custom-fields` + registro `seo_*` en `blog_author` en el mismo commit | JSON-LD del perfil sigue `Thing` (ADR 0037); sin copy sembrado (D-08 / #5 sigue pendiente) |
| META-005 | **Cumplida** 2026-09-03 (plugin 0.7.5): round-trip REST real de la cabeza y de los datos del evento en `tests/WordPress/Editor_SeoPanelTest.php` | Tests de persistencia de **ese** camino, no `meta_input` del importador |
...
**v1.34 (2026-09-23):** OWN-036 / ADR 0046. El WordPress de
