...
- **Inicio (comunidad + evento):** desde 768px, el texto de «Un poco de nuestra comunidad» y la columna destacada forman una fila a **ancho del contenedor** (el mismo marco que el hero). Esa columna lleva la nota del evento y, debajo, los artículos marcados como destacados. Columna de lectura a la izquierda (`65ch`); columna destacada `18.75rem` (WordPress `medium`) **alineada al borde derecho del hero** (`justify-content: space-between`). El rótulo «Próximo evento» va **arriba del cartel**, a la altura del `h2` de comunidad. El cartel se muestra completo al ancho de esa columna, con filete izquierdo, no una caja. Las fotos de esa sección no entran en esa fila: siguen a ancho de lectura, debajo. En móvil, una sola columna en el orden del DOM (texto → rótulo → cartel → datos → fotos), filete superior. No usar `position: sticky`. El peso visual del evento no debe superar al del texto de comunidad.
