...
1. **TDD en dominio nuevo.** Toda política, orquestación o contrato observable del plugin (y
   cualquier helper puro del theme) se escribe RED → el cambio de producción más pequeño →
   REFACTOR. Un bug empieza por un test de regresión. No se escribe una batería especulativa
   de arquitectura que aún no existe.
2. **Tres niveles, un gate barato.** Nivel 1: PHPUnit unitario (`tests/Unit/`), sin base de
...
3. **El plugin posee el dominio; el theme FSE solo ensambla.** CPTs, taxonomías, meta, roles,
...
4. **No fingir WordPress.** Si el objeto de la prueba es un CPT, meta, rewrite o
...
5. **Kit el día uno del PHP propio.** El `composer.json` de raíz, `tests/Unit/`,
   `tests/WordPress/` y `phpunit-wp.xml.dist` se crean **el mismo día** que exista la primera
   línea de plugin o theme — no antes (Fase 3 no ha empezado) y no «cuando el harness no
   baste». Stylelint (`npm run lint:css`) sigue siendo el gate del CSS estático.
6. **CI de calidad ≠ despliegue.** Cuando existan tests PHP, `.github/workflows/test.yml`
...
7. **SonarQube Cloud no es un nivel de PHPUnit.** Automatic Analysis permanece ON. El alcance
   se declara en `.sonarcloud.properties` en el default branch. No se añade
   `sonar-project.properties` ni SonarScanner en CI mientras Automatic Analysis esté ON.
   Cobertura 0.0 % es el comportamiento esperado de Automatic Analysis, no una suite ausente.
8. **WPCS sigue siendo estilo, no prueba.** PHPCS/WPCS (ADR 0027) es el ruleset de código PHP
...
| Instalar Jest/Playwright para el JS estático actual | Descartada (YAGNI): cuatro scripts de front se verifican en navegador; no hay seam que Stylelint + revisión no cubran |
...
- Sonar deja de tratar HTML editorial, `vendor/` y (en el futuro) Core como código propio
  cuando el alcance esté en `main`.
- El gate futuro de PR es barato y no arrastra Docker.

**Contrapartidas aceptadas**

- Hasta Fase 3 no hay `composer test` que ejecutar: el kit no se adelanta a la reorg
  `static/` + `wordpress/`.
...
- ADR [0024](0024-plugin-dominio-theme-presentacion.md), [0027](0027-estandares-ingenieria-codigo.md),
