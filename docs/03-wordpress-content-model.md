...
| share_whatsapp / share_x / share_threads | textarea | Mensaje que el diálogo «Compartir» propone en cada red (WU-08A). Texto plano; `{{SHARE_URL}}` se sustituye por la URL al abrir el diálogo. Vacío = título + URL (en un evento, tipo + nombre). Las mismas tres claves existen en `post`. En wp-admin el panel **Compartir** las edita y las previsualiza, en la entrada y en el evento ([#39](https://github.com/refo44/demo-caminodeldharma/issues/39)). |
...
*(OWN-020 / D-08, 2026-09-01; implementado 2026-09-24, plugin 0.7.7 / theme 0.6.1, [#5](https://github.com/refo44/demo-caminodeldharma/issues/5).)*
...
ensayo largo del fundador. No `noindex` en el single. Payload `blog_authors` lleva `seo`,
contenido y thumbnail. Un import previo converge con `migrate convert`, add-only.
