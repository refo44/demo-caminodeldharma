...
| `/author/zheng-gong` *(nueva)* | *no existe en static* | ficha | `blog_author` | `/author/zheng-gong` | `single-blog_author.html` | `main.js` | `foto-biografia-fundador.jpg` (OWN-020) | seed CPT + `seo` + byline corto + thumb (plugin 0.7.7, [#5](https://github.com/refo44/demo-caminodeldharma/issues/5)); `convert` add-only si la ficha ya existe | 200; `query_var` ≠ `author`; no es user; `index,follow` + meta description publicada |
| `/author/comunidad-camino-del-dharma` *(nueva)* | *no existe en static* | ficha | `blog_author` | `/author/comunidad-camino-del-dharma` | `single-blog_author.html` | `main.js` | `comunidad-quienes-somos.jpg` (OWN-020) | seed CPT + `seo` + primer párrafo de «Quiénes somos» + thumb (plugin 0.7.7, [#5](https://github.com/refo44/demo-caminodeldharma/issues/5)); `convert` add-only si la ficha ya existe | 200; mismo tipo que Zheng Gong; `index,follow` + meta description publicada |
...
| `sessionStorage` de `wp-emoji` | presente — Pass (local) *(delta aceptado OWN-022)* | OWN-022: aceptar; no desactivar `wp-emoji` | ninguno |
