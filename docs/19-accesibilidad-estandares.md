...
| **Calendario de Eventos** | Días de evento: enlace al ancla de la tarjeta. Lunes sin otro evento: `.has-practice`, enlace a `/practica/meditacion-semanal-en-linea`. Nombre accesible: `aria-label` (día + destino; WCAG 2.4.4). Tooltip visual: `data-tooltip` en el enlace, visible en `:hover` y `:focus-visible` en escritorio; en vista táctil o viewport ≤767px, etiqueta corta (`data-tooltip-short`: categoría del evento o «Meditación») siempre visible en la celda y un solo toque sigue el enlace; **no** usar `title` nativo (aviso duplicado y tardío). El número visible no basta como nombre accesible. |
...
**Versión:** 1.9 — Calendario móvil: `data-tooltip-short` = categoría del evento o «Meditación».
