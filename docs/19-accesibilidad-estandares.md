...
| **Calendario de Eventos** | Días de evento: enlace al ancla de la tarjeta. Lunes sin otro evento: `.has-practice`, enlace a `/practica/meditacion-semanal-en-linea`. Nombre accesible: `aria-label` (día + destino; WCAG 2.4.4). Tooltip visual: `data-tooltip` en el enlace, visible en `:hover`, `:focus-visible` y, si no hay hover o el viewport es ≤767px, al primer toque (clase `.is-tooltip-visible`; el segundo toque sigue el enlace); `.eventos-calendar-hint` bajo la cuadrícula (solo móvil/táctil) indica el doble toque; **no** usar `title` nativo (aviso duplicado y tardío). El número visible no basta como nombre accesible. |
...
**Versión:** 1.10 — Eventos: aviso `.eventos-calendar-hint` para doble toque en móvil.
