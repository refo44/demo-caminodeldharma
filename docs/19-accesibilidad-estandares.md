...
- **Idioma:** Atributo `lang` en `<html>`; consistencia de idioma en las páginas; `lang` en fragmentos en otro idioma si aplica. Las formas tibetanas romanizadas usan `lang="bo-Latn"` y `translate="no"`. Pueden incluir una guía silábica secundaria mediante `<ruby>` y `<rt>`, presentada en un `<aside>` identificado y acompañada por un `<dl>` que diferencie origen, forma presentada y sistema de escritura. Como la pronunciación depende de las voces instaladas, se ofrece audio como referencia y no se intenta controlarla mediante `aria-label`.
...
| **Calendario de Eventos** | Días de evento: enlace al ancla de la tarjeta. Lunes sin otro evento: `.has-practice`, enlace a `/practica/meditacion-semanal-en-linea`. Nombre accesible: `aria-label` (día + destino; WCAG 2.4.4). Tooltip visual: `data-tooltip` en el enlace, visible en `:hover` y `:focus-visible`. Con puntero grueso, el primer toque revela el nombre (`.is-tooltip-visible`); «Toca de nuevo para ver el evento.» aparece bajo la cuadrícula con `aria-hidden="true"`; el segundo toque sigue el enlace. Teclado: Enter sigue el enlace al primer paso (no interceptar `event.detail === 0`). **no** usar `title` nativo (aviso duplicado y tardío). El número visible no basta como nombre accesible. |
...
| **Visor ampliado de la galería** | **No existe en la maqueta estática y no se implementará** (ADR 0021): llega con el lightbox nativo del bloque de Gutenberg, que ya gestiona foco, cierre con `Esc` y ARIA. Si alguna vez se hiciera uno propio, debe seguir el patrón `.share-dialog` de `share.js` y cumplir los criterios de diálogo de este documento. |
...
**Versión:** 1.13 — Eventos: aviso táctil `aria-hidden`; Enter sin doble activación.
