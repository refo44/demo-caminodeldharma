...
- **Idioma:** Atributo `lang` en `<html>`; consistencia de idioma en las páginas; `lang` en fragmentos en otro idioma si aplica. Las transliteraciones tibetanas en alfabeto latino usan `lang="bo-Latn"` y `translate="no"`. Pueden incluir una guía silábica secundaria mediante `<ruby>` y `<rt>`. Como la pronunciación depende de las voces instaladas, se ofrece audio como referencia y no se intenta controlarla mediante `aria-label`.
